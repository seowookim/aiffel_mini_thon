...
# 변수들을 sidebar에서 입력받기
with st.sidebar:
    st.session_state.threshold = st.slider("Threshold", 0.0, 1.0, 0.55)
    st.session_state.max_length = st.number_input("Max Length", value=45, step=1)
    st.session_state.min_new_tokens = st.number_input("Min New Tokens", value=3, step=1)
    st.session_state.use_cache = st.checkbox("Use Cache", value=True)
    st.session_state.repetition_penalty = st.number_input("Repetition Penalty", value=4.0, step=0.1)
    st.session_state.do_sample = st.checkbox("Do Sample", value=False)
    st.session_state.num_beams = st.number_input("Num Beams", value=2, step=1)
    st.session_state.temperature = st.number_input("Temperature", value=2.0, step=0.1)
    st.session_state.top_k = st.number_input("Top K", value=50, step=1)
    st.session_state.top_p = st.number_input("Top P", value=0.9, step=0.1)
...
# 저장된 모델 및 토크나이저 로드
cls_path = './models/kogpt2-classification_best'
...
# 저장된 모델 및 토크나이저 로드
gen_path = './models/kogpt2-chatbot_best'
...
if user_input := st.chat_input("무엇이 궁금하신가요?"):
    ...
    # 분류 결과 추론
    predicted_labels = predict_listener_empathy(user_input, trained_cls_model, trained_cls_tokenizer,  st.session_state.threshold)
